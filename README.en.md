...
### Alimony in minimum wages and overdue updates, for AI

**Alimony calculation** for AI. Convert alimony set in minimum wages to the current value and update overdue installments with correction and interest. Live minimum wage and indices, no login.

- 👨‍👩‍👧 **Converts alimony** in minimum wages and updates arrears
...
**Manual** (if the deeplink does not open): [claude.ai/customize/connectors](https://claude.ai/customize/connectors?surface=cowork) → **+** → **Add custom connector** → paste **Name** `Cálculo de Pensão Alimentícia` and **URL** `https://api.mcp.ai/p_calculo-pensao-alimenticia`.

### Cursor

[➕ Install Cálculo de Pensão Alimentícia in Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=calculopensaoalimenticia&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF9jYWxjdWxvLXBlbnNhby1hbGltZW50aWNpYSJ9)

### VS Code (Copilot Chat)

[➕ Install Cálculo de Pensão Alimentícia in VS Code](vscode:mcp/install?name=calculopensaoalimenticia&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_calculo-pensao-alimenticia%22%7D)
...
```
Convert alimony of 30% of the minimum wage to today's value
Update 6 overdue alimony installments with correction and interest
```
...
MIT — see [LICENSE](LICENSE). The MCP server at `api.mcp.ai/p_calculo-pensao-alimenticia` is proprietary (hosted); this repo (manifests, docs, skills) is MIT.
